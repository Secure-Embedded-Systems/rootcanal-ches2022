...
DEBUG = False
REPORT= False
aca_results_file = '../aca/combinedresults-sum_run3.yaml'
correlation_or_welcht = 'welcht'
pc_log_dir = '../sim/pc_log/'
assembly_file = '../sim/software/byte_mask_aes.dump'
FREQ = 5e7 # 50MHz
pc_log_timescale = 1e-12

TOPLEVEL = 'skivav'
MODULE = 'skivav_swaes_bytemasked_tvla'
            ...
            if path_found:
                ...
                if stage == 'Decode Stage': # check if it is from operand forwarding
                    ...
                    for in_edge in netlist_graph.in_edges(gate_output):
                        # if in_edge in pipe_reg_output_stage_dict.keys():   
                        #for bit in range(pipe_width['writeback']):
                        for pipe_reg_node in detected_pipereg_output_names['writeback']:
                            #pipe_reg_node = pipe_reg_output_stage_names['writeback']+'['+str(bit)+']'
                            #if not (pipe_reg_node in netlist_graph.nodes()):
                            #    continue         
